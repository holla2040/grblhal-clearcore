...
- [ ] grblHAL networking plugin: Telnet first, then WebSocket, then HTTP + WebUI (WebUI assets need Phase 7 FS — may land after SD) — **Telnet DONE 2026-08-04**: plugin submodule at src/networking pinned `894a153d`, `enet.c` adapted from iMXRT1062 reference (GPLv3 header retained; settings for services/hostname/ip-mode/ip/gw/mask/telnet-port, DHCP+static, link tracking @250 ms, status events). WebSocket + HTTP/WebUI remain.
