...
- [ ] Add `LICENSE` — **GPLv3** (required: grblHAL core is GPLv3 and Phase 8 upstreams to the org). Every file adapted from ClearCore-library KEEPS Teknic's MIT copyright header plus a one-line "adapted from …, changes: …" note (MIT license condition); new files carry GPLv3 headers per grblHAL convention.
- [ ] Add `.gitignore`: `.pio/`, build artifacts (`*.o`, `*.elf`, `*.bin` — EXCEPT the bootloader dump below), editor droppings
- [ ] `platformio.ini`: upstream `platform = atmelsam`, **no `framework` line** (bare build), `board = teknic_clearcore`. Frameworkless mechanics — this is the wiring that makes it actually build: `board_build.ldscript = ld/flash_with_bootloader.ld` in platformio.ini (the vendored board JSON's `build.arduino.ldscript` key is Arduino-framework-only and does nothing in a bare build); put `startup_same53.c`/`system_same53.c` under `src/` (PlatformIO auto-compiles `src/` in bare builds); `build_flags = -I<DFP include dirs> -D__SAME53N19A__ -D__SAME53__`
- [ ] Vendor `boards/teknic_clearcore.json` + `debug/ATSAME53N19A.svd` + salvaged OpenOCD cfgs + the 2021 Makefile's flash/reset recipes
- [ ] Vendor CMSIS + SAME53 DFP headers from **Microchip packs** (packs.download.microchip.com → `SAME53_DFP` for `same53n19a.h`/component headers, ARM `CMSIS` pack for `core_cm4.h` etc.; Adafruit's CMSIS-Atmel Arduino package is an alternative source). **The Teknic clone does NOT contain these** — its sources `#include <sam.h>` expecting an installed Atmel Studio DFP; the "vendor from Teknic clone" route is a dead end.
- [ ] Copy + adapt `startup_same53.c`, `system_same53.c` (into `src/`), `flash_with_bootloader.ld` (into `ld/`; ORIGIN 0x4000)
- [ ] Add grblHAL core as git submodule, **pinned to a recorded master commit SHA** (record here: ______ )
- [ ] Create `RESOURCES.md` (peripheral/IRQ/GCLK/DMA/EVSYS claim table — starts empty)
...
- [ ] **EXIT TEST:** minimal `main()` builds, links at 0x4000, flashes via Atmel-ICE (sector erase), debugger halts in `main()`, bootloader still enumerates after double-tap reset

### Phase 1 — Bare-metal board bring-up (est. 12–24 h)
- [ ] Clock tree to 120 MHz (adapt system_same53.c); verify by scoping a GPIO toggle or GCLK output
- [ ] Our SysTick @ 1 ms, priority 7: elapsed-ms counter (`hal.get_elapsed_ticks` source) + housekeeping dispatch
- [ ] Shift-register driver (protocol from ShiftRegister.cpp + schematic; pins + full 32-bit map in **Appendix A**; refresh from SysTick context) — **motion-critical, not cosmetic**: motor enables (bits 8–11), COM-port modes (bits 12/13/16/17), and analog-input muxing (bits 28–31) all live here, plus LEDs
- [ ] UART console on COM-0 SERCOM, TX/RX ring buffers, 115200. **Physical layer facts:** COM-0 is an **RJ-45 jack** carrying **5V TTL** (or inverted-TTL "RS-232-compatible" mode); mode + polarity are gated by **shift-register bits** (`SR_UART_TTL_0_MASK`, ShiftRegister.h:148) — **the UART is dead until the shift-register driver (previous task) sets those bits**. Extract the RJ-45 pinout from the schematic into README; use a 5V-tolerant USB-UART adapter.
...
- [ ] `driver.c`/`driver.h` from arm-driver template, wired against CURRENT hal API (2021 code predates: `io_stream_t` + stream-connect registration with `enqueue_realtime_command` RX interception; `spindle_register()`; current NVS signatures; `hal.f_step_timer`, `hal.rx_buffer_size`, `hal.irq_*`; pin enumeration for `$pins`)
- [ ] `my_machine.h` at driver root (board selection, N_AXIS, enabled-plugin switches) — required from day one, not Phase 8: terjeio's guidance in core#10 names it as part of the minimum port set, and every reference driver has it at root
- [ ] UART as first grblHAL stream (real `rx_free` accounting — 2021 faked it)
- [ ] NVS: our NVMCTRL driver on last flash pages (grblHAL standard pattern; NvmManager as reference only)
- [ ] Port atomic helpers from 2021 driver.c
...
- [ ] **Schematic verification before first pulse:** confirm the B-pin→buffer→connector step path is direct GPIO-drivable, determine the role of `Mtr_CLK_01` (PA27) / `Mtr_CLK_23` (PB23) in that path, and confirm signal polarity/inversion through the buffers
...
**Consequences:** (1) `hal.stepper.enable` drives shift-register bits, not GPIO — the Phase 1 shift-register driver is **motion-critical**. (2) HLFB lines all have free EXTINT lines → interrupt-capable fault detection. (3) `Mtr_CLK_01` = PA27, `Mtr_CLK_23` = PB23 exist in the map — **Phase 3 must confirm from the schematic what role these clocks play in the step-output path** (possibly clocking external pulse-shaping logic between the B pin and the connector) before assuming plain GPIO toggling of B reaches the motor. (4) Teknic routes HLFB PWM capture via per-motor EVSYS channels — reference for the optional HLFB-duty feature later.
